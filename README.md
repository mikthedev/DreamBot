...
3. **Allow-list** — nothing is proposed unless an admin **allows** that game. Blocked games are still recorded, never auto-suggested (Warframe can be popular and still stay off-limits). **Add game** looks up **Steam first**, then an **official website** (Minecraft → minecraft.net, Genshin → Hoyoverse); Wikipedia is only a fallback. Use **Fix link** / **Fill empty links** for games already in the list. Sessions are created from catalog games only (Overview dropdown), not free-typed names.
4. **Suggestion** — a message in the configured channel: game, next Saturday evening (default 19:00, Kyiv time), I'm in / Nope. Playing the game does **not** put you on the list. Steam **UA price** (including sales) is filled in automatically. In **Setup**, **Preview message** / **Edit message** customize that card (`{game}` `{when}` `{count}` `{in}` `{link}` `{price}` `{note}` …).
