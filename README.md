...
4. **Suggestion** — a message in the configured channel: game, next Saturday evening (default 19:00, Kyiv time), I'm in / Nope. Playing the game does **not** put you on the list. In **Setup**, **Preview message** / **Edit message** customize that card (`{game}` `{when}` `{count}` `{in}` `{link}` `{note}` …).
